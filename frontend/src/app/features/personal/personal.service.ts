...
export class PersonalService {
  ...
  readonly rows = computed(() =>
    this.selected().filter(
      (e) => e.date.startsWith(this.month()) && (!this.unpaidOnly() || this.due(e) > 0),
    ),
  );
  readonly totals = computed(() => {
    ...
    const sum = (items: { amount: number }[]) =>
      items.reduce((n, e) => n + Math.round(e.amount * 100), 0) / 100;
    const opening =
      sum(entries.filter((e) => e.date < start)) - sum(payments.filter((p) => p.date < start));
  ...
  readonly breakdown = computed(() => {
    ...
    return this.categories
      .map((name) => ({
        name,
        amount: entries.filter((e) => e.category === name).reduce((n, e) => n + e.amount, 0),
      }))
      .filter((c) => c.amount > 0)
      .sort((a, b) => b.amount - a.amount);
  });
  readonly outstanding = computed(() => this.selected().filter((e) => this.due(e) > 0));
  due(e: Expense): number {
    return Math.max(
      0,
      Math.round(
        (e.amount -
          this.data()
            .payments.filter((p) => p.expenseId === e.id)
  ...
  productName(id?: number): string {
    return this.data().products.find((p) => p.id === id)?.name ?? 'General expense';
  }
  expense(id: number): Expense | undefined {
  ...
  async init(): Promise<void> {
    ...
    try {
      ...
      const u = await firstValueFrom(this.api.personalSession());
      this.user.set(u.name);
      await this.load();
    } catch (e: any) {
      if (e.status === 401) this.clearSession();
      else this.error.set(readApiError(e, 'Cannot connect to your account.'));
    } finally {
      this.ready.set(true);
    }
  }
  async authenticate(name: string, password: string, register: boolean): Promise<void> {
    await this.run(async () => {
      const u = await firstValueFrom(this.api.personalAuth(name, password, register));
      this.tokens.setSession(u.accessToken, u.refreshToken, u.name);
      this.user.set(u.name);
      await this.load();
    });
  }
  ...
  private clearSession(): void {
    this.tokens.clear();
    this.user.set('');
    this.data.set({ products: [], expenses: [], payments: [] });
    this.receipt.set(null);
    this.search.set('');
    this.category.set('');
    this.product.set(0);
    this.unpaidOnly.set(false);
  }
  private async refreshSaved(): Promise<void> {
    try {
      ...
      this.error.set(
        'Saved successfully, but the list could not refresh. Reload the page; do not submit again.',
      );
    }
  }
  async load(): Promise<void> {
    ...
    try {
      this.data.set(await firstValueFrom(this.api.personalData()));
    } finally {
  ...
  async run(action: () => Promise<void>): Promise<boolean> {
    ...
    try {
      ...
      this.error.set(readApiError(e, 'Could not save. Please try again.'));
      return false;
    } finally {
      this.busy.set(false);
    }
  ...
  async addExpense(draft: ExpenseDraft): Promise<boolean> {
    return this.run(async () => {
      const e = await firstValueFrom(this.api.personalExpense(draft));
      await this.refreshSaved();
      this.month.set(e.date.slice(0, 7));
      this.message.set('Expense saved. Your hisab is up to date.');
      if (draft.paid > 0)
        this.receipt.set(this.data().payments.find((p) => p.expenseId === e.id) ?? null);
    });
  }
  async addProduct(name: string, unit: string, rate: number): Promise<boolean> {
    return this.run(async () => {
      await firstValueFrom(this.api.personalProduct(name, unit, rate));
      await this.refreshSaved();
      this.message.set('Product added. Select it when recording an expense.');
    });
  }
  async pay(id: number, date: string, amount: number, mode: string): Promise<boolean> {
    return this.run(async () => {
      const p = await firstValueFrom(this.api.personalPay(id, date, amount, mode));
      await this.refreshSaved();
      this.receipt.set(p);
      this.message.set('Payment recorded. Purchase totals are unchanged.');
    });
  }
  ...
  downloadReceipt(): void {
    const p = this.receipt();
    if (!p) return;
    const e = this.expense(p.expenseId);
    if (!e) return;
    ...
    a.href = url;
    a.download = `hisab-payment-${p.id}.html`;
    a.click();
